...
namespace Component.TimerTask.DAL.DBUtility
{
    /// <summary>
    ...
    public class SQLiteHelper
    {
        ...
        public static DataSet ExecuteDataset(SQLiteTransaction transaction, string commandText, object[] commandParameters)
        {
            ...
            IDbCommand cmd = transaction.Connection.CreateCommand();
            cmd.CommandText = commandText;
            SQLiteParameterCollection coll = DeriveParameters((SQLiteCommand)cmd, commandParameters);
        ...
        public static int ExecuteNonQuery(SQLiteTransaction transaction, string commandText, object[] paramList)
        {
            ...
            IDbCommand cmd = transaction.Connection.CreateCommand();
            cmd.CommandText = commandText;
            SQLiteParameterCollection coll = DeriveParameters((SQLiteCommand)cmd, paramList);
            //foreach (SQLiteParameter parm in coll)
            //    cmd.Parameters.Add(parm);
            if (transaction.Connection.State == ConnectionState.Closed)
                transaction.Connection.Open();
        ...
        public static SQLiteParameterCollection DeriveParameters(SQLiteCommand command, object[] paramList)
        {
            if (paramList == null) return null;

            SQLiteParameterCollection coll = command.Parameters;
            string parmString = command.CommandText.Substring(command.CommandText.IndexOf("@"));
